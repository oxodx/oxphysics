...
public class PressureGenerator {
  ...
  public static void step(MinecraftSpace space) {
    ...
    for (var rigidBody : space.getRigidBodiesByClass(ElementRigidBody.class)) {
      if (!rigidBody.isActive() || (rigidBody.getBuoyancyType() == ElementRigidBody.BuoyancyType.NONE
          && rigidBody.getDragType() == ElementRigidBody.DragType.NONE)) {
      ...
      if (linearVelocity.length() < rigidBody.getLinearSleepingThreshold()
          && angularVelocity.length() < rigidBody.getAngularSleepingThreshold()) {
      ...
      for (var triangle : triangles) {
        ...
        final var tangentialVelocity = new Vector3f(angularVelocity).cross(centroid); // angular velocity converted to
                                                                                      // linear parallel to edge of
                                                                                      // circle (tangential)
        final var netVelocity = new Vector3f(tangentialVelocity).addLocal(linearVelocity); // total linear + tangential
                                                                                           // velocity
        ...
        chunkCache.getFluidColumn(blockPos).ifPresent(fluidColumn -> {
          final var waterHeight = fluidColumn.getTop().blockPos().getY()
              + fluidColumn.getTopHeight(posRelativeToBlockCenter) - location.y - centroid.y;
      ...
      for (var triangle : triangles) {
        ...
        if (submergedTriangles.contains(triangle)) {
          final var posRelativeToBlockCenter = new Vector3f(centroid).add(location)
              .subtract(Convert.toBullet(blockPos));

          final var waterHeight = chunkCache.getFluidColumn(blockPos)
              .map(fluidColumn -> (float) fluidColumn.getTop().blockPos().getY()
                  + fluidColumn.getTopHeight(posRelativeToBlockCenter) - location.y - centroid.y)
              .orElse(0.0f);

          chunkCache.getFluidColumn(new BlockPos((int) location.x, (int) location.y, (int) location.z))
              .ifPresent(fluidColumn -> {
          ...
          if (rigidBody.isWaterDragEnabled()) {
            final var tangentialVelocity = new Vector3f(angularVelocity).cross(centroid); // angular velocity converted
                                                                                          // to linear parallel to edge
                                                                                          // of circle (tangential)
            final var netVelocity = new Vector3f(tangentialVelocity).addLocal(linearVelocity); // total linear +
                                                                                               // tangential velocity

            if (Math.signum(netVelocity.dot(area)) == 1) {
              final var dragForce = new Vector3f(area)
                  .multLocal(-0.5f * dragCoefficient * WATER_DENSITY * netVelocity.lengthSquared());
               ...
               * touching water :(
               */
              final var stoppingForce = new Vector3f(netVelocity)
                  .multLocal(-1.0f * rigidBody.getMass() * crossSectionalAreas.get(triangle) / totalArea)
          ...
          // final var pressure = (float) (SEA_LEVEL_PRESSURE * Math.exp(MOLAR_MASS_OF_AIR
          // * gravity.y * (SEA_LEVEL - location.y - centroid.y) / (GAS_CONSTANT *
          // TEMPERATURE)));
          // final var buoyantForce = new Vector3f(area).multLocal(pressure);
          //
          // if (Float.isFinite(buoyantForce.lengthSquared()) &&
          // buoyantForce.lengthSquared() > 0.0f) {
          ...
          if (rigidBody.isAirDragEnabled()) {
            ...
            final var airDensity = (float) (AIR_DENSITY * Math.exp(
                MOLAR_MASS_OF_AIR * gravity.y * (SEA_LEVEL - location.y - centroid.y) / (GAS_CONSTANT * TEMPERATURE)));
            ...
            if (Math.signum(netVelocity.dot(area)) == 1) {
              final var dragForce = new Vector3f(area)
                  .multLocal(-0.5f * dragCoefficient * airDensity * netVelocity.lengthSquared());
              dragForce.multLocal(-1.0f * Math.signum(netVelocity.dot(dragForce))); // make sure all the vectors are
                                                                                    // facing the same way
      ...
      if (rigidBody.getDragType() == ElementRigidBody.DragType.SIMPLE) {
        ...
        final var dragForce = new Vector3f(linearVelocity.normalize())
            .multLocal(-0.5f * area * dragCoefficient * AIR_DENSITY * linearVelocity.lengthSquared());
  ...
  public static float getAddedMassForceAdjustment(List<Triangle> triangles, float mass) {
    ...
    final var sum = triangles.stream()
        .mapToDouble(triangle -> triangle.getArea().length() * triangle.getCentroid().length()).sum();
