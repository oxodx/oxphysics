package nl.oxod.oxphysics.bullet.collision.space;
...
import nl.oxod.oxphysics.api.event.collision.ElementCollisionEvents;
...
import nl.oxod.oxphysics.bullet.collision.space.cache.ChunkCache;
 ...
 * update
 * happens asynchronously while all of the setup, input, or otherwise user
 * defined
 * behavior happens on the game logic thread.
 * <br>
 * <br>
 * It is also worth noting that another
 * simulation step will not be performed if the last step has taken longer than
 * 50ms and is still executing upon the
 * next tick. This really only happens if you are dealing with an ungodly amount
 * of rigid bodies or your computer is slo.
 * 
 * @see PhysicsThread
...
public class MinecraftSpace extends PhysicsSpace implements PhysicsCollisionListener {
  ...
  public MinecraftSpace(PhysicsThread thread, Level level) {
    super(
        // new Vector3f(-Level.MAX_LEVEL_SIZE, Level.MIN_ENTITY_SPAWN_Y,
        // -Level.MAX_LEVEL_SIZE),
        // new Vector3f(Level.MAX_LEVEL_SIZE, Level.MAX_ENTITY_SPAWN_Y,
        // Level.MAX_LEVEL_SIZE),
  ...
  public void collision(PhysicsCollisionEvent event) {
    ...
    /* Element on Element */
    if (event.getObjectA() instanceof ElementRigidBody rigidBodyA
        && event.getObjectB() instanceof ElementRigidBody rigidBodyB) {
      ElementCollisionEvents.ELEMENT_COLLISION.invoker().onCollide(rigidBodyA.getElement(),
          rigidBodyB.getElement(), impulse);

      /* Block on Element */
    } else if (event.getObjectA() instanceof TerrainRigidBody terrain
        && event.getObjectB() instanceof ElementRigidBody rigidBody) {
      ElementCollisionEvents.BLOCK_COLLISION.invoker().onCollide(rigidBody.getElement(), terrain, impulse);

      /* Element on Block */
    } else if (event.getObjectA() instanceof ElementRigidBody rigidBody
        && event.getObjectB() instanceof TerrainRigidBody terrain) {
      ElementCollisionEvents.BLOCK_COLLISION.invoker().onCollide(rigidBody.getElement(), terrain, impulse);
    }
  }
}
