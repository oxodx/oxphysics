...
public class Triangle {
  ...
  public static List<Triangle> getMeshOf(VoxelShape voxelShape) {
    ...
    for (var box : aabbs) {
      ...
      vec.set((float) center.x, (float) center.y, (float) center.z)
          .subtractLocal(0.5f, 0.5f, 0.5f);
  ...
  public static void createBoxMesh(final float x, final float y, final float z,
      final Vector3f offset, Consumer<Triangle> consumer) {
  ...
  public Triangle(Vector3f v1, Vector3f v2, Vector3f v3) {
    ...
    this.area.multLocal(Math.signum(centroid.dot(area)));
  }
