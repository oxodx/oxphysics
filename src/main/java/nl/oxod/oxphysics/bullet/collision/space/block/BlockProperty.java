package nl.oxod.oxphysics.bullet.collision.space.block;
...
public record BlockProperty(float friction, float restitution, boolean collidable, boolean isFullBlock) {
  ...
  public static void addBlockProperty(Block block, float friction, float restitution, boolean collidable,
      boolean isFullBlock) {
    blockProperties.put(block,
        new BlockProperty(Math.max(friction, 0.0f), Math.max(restitution, 0.0f), collidable, isFullBlock));
  }
