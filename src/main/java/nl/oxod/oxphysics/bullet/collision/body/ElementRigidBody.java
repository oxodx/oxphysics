package nl.oxod.oxphysics.bullet.collision.body;
...
import nl.oxod.oxphysics.bullet.thread.util.Clock;
import nl.oxod.oxphysics.util.Frame;
...
public abstract class ElementRigidBody extends MinecraftRigidBody {
  ...
  public ElementRigidBody(PhysicsElement element, MinecraftSpace space, MinecraftShape shape, float mass,
      float dragCoefficient, float friction, float restitution) {
  ...
  public void readTagInfo(ValueInput view) {
    view.read("orientation", ExtraCodecs.QUATERNIONF_COMPONENTS).map(Convert::toBullet)
  ...
  public boolean isWaterDragEnabled() {
    // We check for simple drag here, but complex drag is always used for water
    // buoyancy.
