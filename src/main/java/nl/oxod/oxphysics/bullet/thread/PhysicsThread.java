package nl.oxod.oxphysics.bullet.thread;
...
import nl.oxod.oxphysics.bullet.collision.space.supplier.level.LevelSupplier;
...
import nl.oxod.oxphysics.bullet.collision.space.supplier.entity.EntitySupplier;

/**
 * In order to access an instance of this, all you need is a {@link Level} or
 * {@link ReentrantBlockableEventLoop} object.
 * Calling {@link PhysicsThread#execute} adds a runnable to the queue of tasks
 * and is the main way to execute code on
 * this thread. You can also execute code here by using
 * {@link PhysicsSpaceEvents}.
 * 
...
public class PhysicsThread extends Thread implements Executor {
  ...
  public PhysicsThread(Executor parentExecutor, Thread parentThread, LevelSupplier levelSupplier,
      EntitySupplier entitySupplier, String name) {
    ...
    OxPhysics.LOGGER.info("Starting " + getName());
    this.start();
  }
  ...
  public void destroy() {
    ...
    try {
      ...
      OxPhysics.LOGGER.error("Error joining " + getName());
      e.printStackTrace();
    }
  }
}
